...
read81xVar <- function(
  ...
  resBlocks <- lapply( seq_along(blockStarts0), function(iChunk){
    ...
    # search for the first line that starts with <ID>: (colon) with id having no tab char
    # in lines of before the next blockstart
    iLines <- blockStart:(blockStarts[iChunk + 1L] - 1L)
    sumStarts <- grep("^[^\t]*:", lines[iLines])
    sumStart <- if (length(sumStarts)) sumStarts[1] else length(iLines)+1 #blockStarts[iChunk + 1L]
